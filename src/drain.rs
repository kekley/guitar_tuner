use crate::circular_buffer::add_mod;
...
impl<'a, T> Drain<'a, T> {
    ...
    unsafe fn read(&self, index: usize) -> T {
        debug_assert!(
            index < self.buf_cap && index < self.buf_size,
        ...
        let index = add_mod(buf.start, index, self.buf_cap);
        ptr::read(buf.items[index].assume_init_ref())
    }

    fn as_slices(&self) -> (&[T], &[T]) {
        if self.buf_cap == 0 || self.buf_size == 0 || self.iter.is_empty() {
            return (&[][..], &[][..]);
        }

        let buf = unsafe { self.buf.as_ref() };

        debug_assert!(buf.start < self.buf_cap, "start out-of-bounds");
        debug_assert!(self.buf_size <= self.buf_cap, "size out-of-bounds");

        let start = add_mod(buf.start, self.iter.start, self.buf_cap);
        let end = add_mod(buf.start, self.iter.end, self.buf_cap);
